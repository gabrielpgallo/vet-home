...
As credenciais da aplicação continuam no ambiente Production da Vercel. A Action não precisa das credenciais Google, Gemini nem do banco de produção. O build final é executado na Vercel com suas variáveis Production.

## Publicar

1. Envie e revise os commits na `main`.
2. Havendo migrations novas, faça backup e aplique-as conforme `DEPLOY.md` antes de publicar. O workflow migra apenas um banco descartável de testes; não altera o Supabase. Migrations devem ser compatíveis com a versão ainda publicada.
